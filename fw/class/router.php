...
class router{
  ...
  function __construct(){
    $this->validRequest = array();
  }
  function get($pathWeb,$whatToCall,$params=array()){
    $this->validRequest[$pathWeb] =new routerCall("GET",$whatToCall,$params);
  }
  function validateRequest(){
    if(array_key_exists($_SERVER["REQUEST_URI"],$this->validRequest)){
      if($_SERVER["REQUEST_METHOD"]===$this->validRequest[$_SERVER["REQUEST_URI"]]->method){
        //add method call
        return new message(200,true,__("All OK"));
      }
    }else{
      $URI = $_SERVER["REQUEST_URI"];
      $slicesURI = preg_split("/\//",$URI);
      $slicesURI = array_filter($slicesURI, array("util",'_remove_empty_internal'));
      $slicesURI = array_values($slicesURI);
      $slicesCount = count($slicesURI);
      $keepForeach = true;
      foreach($this->validRequest as $endpoint=>$routerCall){
        if($keepForeach){
          $slicesURIEndpoint = preg_split("/\//",$endpoint);
          $slicesURIEndpoint = array_filter($slicesURIEndpoint, array("util",'_remove_empty_internal'));
          $slicesURIEndpoint = array_values($slicesURIEndpoint);
          if($slicesCount==count($slicesURIEndpoint)){
            $good = false;
            $vars = array();
            for($i=0;$i<count($slicesURIEndpoint)-1;$i++){
              p($slicesURI[$i]);
              p($slicesURIEndpoint[$i]);
              if($slicesURI[$i]==$slicesURIEndpoint[$i]){
                $good=true;
              }
            }

            if($good){
              util::p($slicesURIEndpoint);
              util::p($endpoint);
              util::p($routerCall);
              preg_match_all('/{(.*?)}/', $endpoint, $outMatch);
      ...
      return new message(404,false,__("Endpoint not found"));
    }
  }
...
class routerCall{
  ...
  var $whatToCall;
  var $params;
  function __construct($method,$whatToCall,$params=array()){
    $this->method      = $method;
